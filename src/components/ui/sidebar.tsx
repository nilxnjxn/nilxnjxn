  ...
  (
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        ...
        // Also save to localStorage for client-side state
      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            className={cn(
              'group/sidebar-wrapper flex min-h-svh w-full flex-row has-data-[variant=inset]:bg-sidebar',
              className,
            )}
            ref={ref}
            {...props}
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "group peer hidden md:flex flex-col text-sidebar-foreground transition-[width] duration-200 ease-linear w-[--sidebar-width] data-[state=collapsed]:w-[--sidebar-width-icon] shrink-0",
          className
        )}
        data-state={state}
...
>(({ className, ...props }, ref) => {
  return (
    <main
      ref={ref}
      className={cn(
        'relative flex min-h-svh flex-1 min-w-0 flex-col bg-background',
